...
本フォークは **上流の機能をすべて維持したうえで「リモートサーバーモード」を追加**します。SSH でリモート Linux サーバーに接続し、リモート側で一時エージェントを実行して、リモートの `~/.cc-switch` データベースを直接読み書きします。リモート上で動く Claude / Gemini / OpenCode / Grok / Codex / Kimi のセッション利用量をデスクトップに取り込んで表示し、ローカルとリモート間でプロバイダーの同期も行えます。
...
- **リモート利用量同期** — 接続成功時に 1 回、Usage ページを開いたときにさらに 1 回自動同期、あとはいつでも手動同期。Claude / Gemini / OpenCode / Grok / Codex / Kimi の 6 ソースのセッションログをリモート DB に取り込み
...
- 読み取るリモートデータ: リモート `$HOME` 配下のセッションファイル — `~/.claude`、`~/.gemini`、`~/.local/share/opencode`、`~/.grok`、`~/.codex`、`~/.kimi-code` — および `~/.cc-switch/` データベース。デスクトップのローカルファイルはアップロードも読み取りもされません
